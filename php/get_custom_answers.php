...
$answer = new ClientAnswer();
 ...
 $question = new CustomizedQuestions(); 
 $specific_questions = $question->fetch_single_question($project_question_id);
 
 
 $answer_details =  $answer->fetch_aprroved_answer_custom($project_question_id, $project_id);
  
  
 ?>
<div class="main-box clearfix">
    <header class="main-box-header clearfix">
        <h3> <?= ucwords($specific_questions["customize_qsn"]) ?> </h3>
    </header>
</div>
...
<?php 
  $answer_details =  $answer->fetch_c_answers($project_question_id, $project_id);
...
?>   
    ...
    <?php if($responser_admin) { ?> <div class="panel panel-info">  <?php } else ?>
    <?php if ($responser_member) { ?> <div class="panel panel-success">  <?php } else  ?>
